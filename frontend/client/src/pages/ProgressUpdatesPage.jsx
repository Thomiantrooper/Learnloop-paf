...
function ProgressUpdatesPage() {
  ...
  const fetchUpdates = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/progress-updates/user/${localStorage.getItem("userId")}`);
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/api/progress-updates/${id}`, { method: "DELETE" });
      if (!response.ok) {
        throw new Error(`Error: ${response.status} ${response.statusText}`);
      }
      fetchUpdates();
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-800">My Learning Dashboard</h1>
            <p className="text-gray-600 mt-2">Track and visualize your learning progress</p>
          </div>
          
          <div className="flex space-x-3">
            {/* <button
              onClick={() => setShowGraph(!showGraph)}
              className="flex items-center space-x-2 px-4 py-2 bg-white text-gray-700 rounded-xl hover:bg-gray-100 transition duration-300 shadow-sm border border-gray-200"
            >
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition duration-300 shadow-md"
            >
  ...
  🎖️ View Skill Insights
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold text-gray-800">My Progress Updates</h2>
